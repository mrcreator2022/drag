...
//Database Connection
$con = mysqli_connect( "bisonent.com.au","bisonent_stand","xKpU7CgTfAYKuChxKpU7CgTfAYKuCh","bisonent_jobtracker") or die(mysqli_error());
...
$previous_week = strtotime($today, "+1 day");
        ...
        <div class="container-fluid" style="height: 90vh;">
            ...
            <div class="row border border-dark mx-1 h-25" >
                <div class="col-12 col-md-6 border border-dark position-relative dropzone">
                    ...
                    <?php
                    $result_mon = mysqli_query($con, "SELECT `quote`, `client`, `company`, `mgr_initial`, `sent_date`, `date_required` FROM `quotes` WHERE `date_required` = '" . date('Y-m-d', strtotime('+1 day', $start_week)) . "'  ") or die(mysqli_error());
                    while($row = mysqli_fetch_array($result_mon)) {
                        if($row['sent_date'] === $row['date_required']) { $background = 'alert-success';} else { $background = '';}
                        echo '<h3 draggable="true" class="box border rounded m-1 px-1 my-1 '.$background.' ">' . floatval($row['quote']) . ' - <b>' . $row['client'] . '</b> ' . $row['company'] . ' - ' . $row['mgr_initial'] . '</h3>';
                    }
                    ?>
                ...
                <div class="col-12 col-md-6 border border-dark position-relative dropzone">
                    ...
                    <?php $result_tue = mysqli_query($con, "SELECT `quote`, `client`, `company`, `mgr_initial`, `sent_date`, `date_required` FROM `quotes` WHERE `date_required` = '" . date('Y-m-d', strtotime('+2 day', $start_week)) . "'    ") or die(mysqli_error());
            ...
            <div class="row border border-dark mx-1 h-25">
                <div class="col-12 col-md-6 border border-dark position-relative dropzone">
                    ...
                    <?php $result_wed = mysqli_query($con, "SELECT `quote`, `client`, `company`, `mgr_initial`, `sent_date`, `date_required` FROM `quotes` WHERE `date_required` = '" . date('Y-m-d', strtotime('+3 day', $start_week)) . "'    ") or die(mysqli_error());
                ...
                <div class="col-12 col-md-6 border border-dark position-relative dropzone">
                    ...
                    <?php $result_thu = mysqli_query($con, "SELECT `quote`, `client`, `company`, `mgr_initial`, `sent_date`, `date_required` FROM `quotes` WHERE `date_required` = '" . date('Y-m-d', strtotime('+4 day', $start_week)) . "'    ") or die(mysqli_error());
            ...
            <div class="row border border-dark mx-1 h-25">
                <div class="col-12 col-md-6 border border-dark position-relative dropzone">
                    ...
                    <?php $result_fri = mysqli_query($con, "SELECT `quote`, `client`, `company`, `mgr_initial`, `sent_date`, `date_required` FROM `quotes` WHERE `date_required` = '" . date('Y-m-d', strtotime('+5 day', $start_week)) . "'    ") or die(mysqli_error());
                ...
                <div class="col-12 col-md-6 border border-dark position-relative">
                    ...
                    <?php $result_fri = mysqli_query($con, "SELECT `quote`, `client`, `company`, `mgr_initial`, `sent_date`, `date_required` FROM `quotes` WHERE ( `follow_up` <= '" . date('Y-m-d', strtotime('-7 day', $start_week)) . "' OR `follow_up` IS NULL ) AND `sent_date` IS NOT NULL AND `stage` IN ('Hot', 'Potential') ORDER BY `quote` DESC LIMIT 10 ") or die(mysqli_error());
                    while($row = mysqli_fetch_array($result_fri)) {
            ...
            <div class="row border border-dark mx-1 h-25">
                <div class="col-12 col-md-6 border border-dark position-relative dropzone">
                    ...
                    <?php $result_fri = mysqli_query($con, "SELECT `quote`, `client`, `company`, `mgr_initial`, `sent_date`, `date_required` FROM `quotes` WHERE `date_required` > '" . date('Y-m-d', strtotime('+5 day', $start_week)) . "' AND `sent_date` IS NULL ") or die(mysqli_error());
                    while($row = mysqli_fetch_array($result_fri)) {
